...
fs.readFile(file, 'utf8', (err, data) => {
  ...
  const serialEnhance = (image,rounds) => {
    let r = image;
  ...
  const trim = image => image.substring(w,image.length-w).split('').filter((c,i) => !(i%w < 60 || i%w > (w-60))).join('');
  const print = image => image.split('').map((c,i) => (i%(w-120) === 0) ? '\n'+c : c).join('');

  console.log("E2: ", count(trim(serialEnhance(original, 50))));
});

...
fs.readFile(file, 'utf8', (err, data) => {
  ...
  const findPaths = (conn, path) => {
    return conn[path[path.length-1]]
        .reduce((acc, cave) => {
          if (cave === 'end') return [...acc, [...path, cave]];
          if (isSmallCave(cave) && path.includes(cave)) return acc;
          return [
            ...acc,
            ...findPaths(conn, [...path, cave])
          ];
        }, []);
  };

  console.log("E1: ", findPaths(connections(input), ['start']).length);
});

...
fs.readFile(file, 'utf8', (err, data) => {
  ...
  const RANK_OF_CARDS = "AKQT98765432J";
  ...
  const type = (hand) => {
    ...
    const j = (k.includes('J')) ? c['J'] : false;

    if (j) {
      if (h === 5) return TYPE_FIVE_OF_KIND;
      if (h === 4) return TYPE_FIVE_OF_KIND;
      if (k.length === 2 && j === 3) return TYPE_FIVE_OF_KIND;
      if (k.length === 3 && j === 3) return TYPE_FOUR_OF_KIND;
      if (h === 3 && j === 2) return TYPE_FIVE_OF_KIND;
      if (h === 3 && j === 1) return TYPE_FOUR_OF_KIND;
      if (h === 2 && k.length === 3 && j === 2) return TYPE_FOUR_OF_KIND;
      if (h === 2 && k.length === 3 && j === 1) return TYPE_FULL_HOUSE;
      if (k.length === 4 && h === 2) return TYPE_THREE_OF_KIND;

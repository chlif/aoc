...
fs.readFile(file, 'utf8', (err, data) => {
  ...
  const rounds = [0, 19, 999, 1999, 2999, 3999, 4999, 5999, 6999, 7999, 8999, 9999];
  const kn = monkeys.reduce((a,b) => a*b.divider, 1);
  ...
  for (let round = 0; round < 10000; round++) {
    for (let i in monkeys) {
      ...
      while (monkeys[i].items.length) {
        const next = monkeys[i].items.shift() % kn;
        monkeys[((next % monkeys[i].divider === 0) ? monkeys[i].dest[0] : monkeys[i].dest[1])].items.push(next);
      }
    }
    
    //if (rounds.includes(round)) console.log(monkeys.map(m => m.handled));
    //if (rounds.includes(round)) console.log((round+1),monkeys.map(m => m.items));
  }

  console.log("E2: ", monkeys.map(m => m.handled).sort((a,b) => b-a).slice(0,2).reduce((a,b) => a*b, 1));
});
...
const buildCalculation = (parts) => {
  if (parts[1] === 'old') {
    return (parts[0] === '+') ? (x) => x+x : (x) => x*x;
  }
  const factor = parseInt(parts[1]);
  return (parts[0] === '+') ? (x) => x+factor : (x) => x*factor;
};
...
fs.readFile(file, 'utf8', (err, data) => {
  ...
  const evaluate = package => {
    switch (package.t) {
      case 4:
        return package.b;
      case 0:
        return package.s.reduce((a,s) => a + evaluate(s), 0);
      case 1:
        return package.s.reduce((a,s) => a * evaluate(s), 1);
      case 2:
        return package.s.map(s => evaluate(s)).reduce((a,x) => (a === undefined || x < a) ? x : a, undefined);
      case 3:
        return package.s.map(s => evaluate(s)).reduce((a,x) => (a === undefined || x > a) ? x : a, undefined);
      case 5:
        return (evaluate(package.s[0]) > evaluate(package.s[1])) ? 1 : 0;
      case 6:
        return (evaluate(package.s[0]) < evaluate(package.s[1])) ? 1 : 0;
      case 7:
        return (evaluate(package.s[0]) === evaluate(package.s[1])) ? 1 : 0;
  ...
  console.log("E2: ", evaluate(parse(hexToBin(input))));
});
